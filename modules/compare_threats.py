...
def text_diff_side_by_side(text1, text2):
    """Generates two HTML blocks for a side-by-side comparison view."""
    try:
        ...
        for tag, i1, i2, j1, j2 in codes:
            if tag == 'equal':
                content = html.escape(text1[i1:i2])
                diff_a += content
                diff_b += content
            elif tag == 'delete':
                diff_a += f'<span class="diff-del">{html.escape(text1[i1:i2])}</span>'
            elif tag == 'insert':
                diff_b += f'<span class="diff-ins">{html.escape(text2[j1:j2])}</span>'
            elif tag == 'replace':
                diff_a += f'<span class="diff-del">{html.escape(text1[i1:i2])}</span>'
                diff_b += f'<span class="diff-ins">{html.escape(text2[j1:j2])}</span>'
...
def _show_compare_mode_internal():
    ...
    if st.session_state.compare_src == "History":
        ...
        def render_history_selector(col, scan_key, label):
            with col:
                ...
                with f_col1:
                    filter_sector = st.selectbox(f"Filter Sector", ["ALL", "URL", "PHISHING", "HASH", "CVE", "LOG", "BREACH"], key=f"f_sec_{scan_key}")
                with f_col2:
        ...
        comp_type = st.pills("Select Forensic Sector", ["URL", "PHISHING", "HASH", "CVE", "LOG", "BREACH"], default="URL")
        ...
        sector_config = {
            ...
            "BREACH": {"placeholder": "Enter Email or Password to check...", "min_len": 3}
        }
                    ...
                    def fetch_live_scan(content, t_type):
                        ...
                        try:
                            if t_type == "URL":
                                ...
                                # Detect if email or password
                                if "@" in content:
                                    res["intel"] = IntelligenceService.simulate_email_breach(content)
                                    res["sub_type"] = "Email"
                                else:
                                    res["intel"] = IntelligenceService.check_pwned_password(content)
                                    res["sub_type"] = "Password"
                            else:
                                res["lexical"] = {"entropy": IntelligenceService.calculate_entropy(content)}
                        except Exception as e:
                            res["status"] = "error"
    ...
    try:
        ...
        delta_metrics = {
            ...
            "Scan A": [
                scan_a.get('type', 'UNK'),
                whois_a.get('registrar', 'N/A') if scan_a.get('type') == "URL" else (res_a.get('sub_type', 'N/A') if scan_a.get('type') == "BREACH" else f"ID: #{scan_a.get('id', '0')}"),
                geo_a.get('country', 'N/A') if scan_a.get('type') == "URL" else (f"{vt_a.get('stats', {}).get('malicious', 0)} Flags" if scan_a.get('type') == "HASH" else (f"{len(res_a.get('intel', {}).get('breaches', []))} Breaches" if res_a.get('sub_type') == "Email" else f"{res_a.get('intel', {}).get('count', 0)} Pwns")),
                f"{ent_a}",
                f"{len_a} ch"
            ],
            "Scan B": [
                get_stat_with_indicator(scan_a.get('type'), scan_b.get('type')),
                get_stat_with_indicator(whois_a.get('registrar'), whois_b.get('registrar')) if scan_a.get('type') == scan_b.get('type') == "URL" else get_stat_with_indicator(res_a.get('sub_type', scan_a.get('id')), res_b.get('sub_type', scan_b.get('id'))),
                get_stat_with_indicator(geo_a.get('country'), geo_b.get('country')) if scan_a.get('type') == scan_b.get('type') == "URL" else get_stat_with_indicator(vt_a.get('stats', {}).get('malicious', -1) if scan_a.get('type') == "HASH" else (len(res_a.get('intel', {}).get('breaches', [])) if res_a.get('sub_type') == "Email" else res_a.get('intel', {}).get('count', 0)), vt_b.get('stats', {}).get('malicious', -1) if scan_b.get('type') == "HASH" else (len(res_b.get('intel', {}).get('breaches', [])) if res_b.get('sub_type') == "Email" else res_b.get('intel', {}).get('count', 0)), "Flags/Hits"),
    ...
    # --- 2. Structural Side-by-Side Diff ---
    with st.expander("🔍 Structural Input Comparison (Side-by-Side)", expanded=True):
        diff_a, diff_b = text_diff_side_by_side(scan_a.get('input', ''), scan_b.get('input', ''))
        
        col_diff_a, col_diff_b = st.columns(2)
        with col_diff_a:
            st.caption("Scan A (Red highlights what was removed/changed)")
            st.markdown(f'<div class="diff-container">{diff_a}</div>', unsafe_allow_html=True)
        with col_diff_b:
            st.caption("Scan B (Green highlights what was added/new)")
            st.markdown(f'<div class="diff-container">{diff_b}</div>', unsafe_allow_html=True)
        
        st.markdown("""
        <div style="font-size: 0.8rem; margin-top: 10px; color: var(--text-secondary);">
            <b>How to read:</b> Compare the red sections in Scan A with the green sections in Scan B to see exactly what the attacker changed.
        </div>
